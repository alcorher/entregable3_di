...
export default function Register() {
  ...
  const validarFormulario = async (e) => {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ email: mail, password: password, nombre: username }),
      });
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4">
      <img src="/images/formsBG.png" className="fixed inset-0 w-full h-full object-cover -z-10" alt="Fondo" />
      <div className="fixed inset-0 bg-black/50 -z-5"></div>

      <div className="w-full max-w-md">
        <img src="/images/logo.png" className="w-full h-auto mx-auto mb-8" alt="Logo" />
          ...
          <div className="bg-white rounded-2xl shadow-md">
            <div className="bg-brand-300 p-8 rounded-2xl shadow-md">
              ...
              <form onSubmit={validarFormulario} className="space-y-6">
                ...
                <div>
                  <label htmlFor="mail" className="block text-sm font-semibold text-brand-900 mb-2">
                    Correo electrónico
                  </label>
                  <input
                    type="email"
                ...
                <div>
                  <label htmlFor="confirmPassword" className="block text-sm font-semibold text-brand-900 mb-2">
                ...
                <a className="text-sm text-start text-gray-900 block hover:underline mb-8" href="/inicioSesion">
                  ¿Ya tienes cuenta? <span className="inline text-brand-900 font-semibold hover:underline">Inicia sesión</span>
                </a>
                ...
                <button
                  ...
                  className={`btn w-full shadow-lg shadow-zinc-600 transition-all duration-200 ease-out focus:outline-none ${
                    loading ? "opacity-50 cursor-not-allowed" : "hover:shadow-md hover:-translate-y-0.5 active:translate-y-0.5 active:shadow-md active:scale-99"
                  }`}
                >
